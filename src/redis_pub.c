...
{
	return id? internal_do_get_postfix_from_TID(cfg("redis.topic"), id, flags) : id;
}
...
static char const * internal_get_CID_from_SID(char const * sid)
{
	...
	sds pattern = sdscatfmt(sdsempty(), "%s:s:", cfg("redis.topic"));
		...
		, redisCallbackFn done)
{
	...
	sds topic = internal_TID_gen(cfg("redis.topic"), id, flags);
		...
		)
{
	...
	sds sid = sdscatfmt(sdsempty(), "%s:s:%s", cfg("redis.topic"), id);

	/* make sure session exits for this id */
	sds idtopic = internal_TID_gen(cfg("redis.topic"), id, 0);
	rc = redisAsyncCommand(c, 0, 0/* privdata */, "hsetnx %s %s %s", sid, idtopic, "0");

	subc = hp_subc_arg(c, subc, cfg("redis.shasub"), cfg("redis.shasup"), sid, cb, arg);
...
int redis_sub(rmqtt_io_t * client, int n_topic, char * const* topic, uint8_t * qoss)
{
	...
	if (topic && n_topic > 0) {
		sds t = internal_TID_gen(cfg("redis.topic"), client->sid, 0);
	...
	for(i = 0; i < n_topic; ++i){
		...
		sds t = internal_TID_gen(cfg("redis.topic"), key, 8);
		cvector_push_back(topics, t);
		/* save QOS */
...
int redis_sub_sadd(redisAsyncContext * c, char const * id, char const * topicstr)
{
	...
	int rc;
	sds sid = sdscatfmt(sdsempty(), "%s:s:%s", cfg("redis.topic"), id);
	sds topic = internal_TID_gen(cfg("redis.topic"), topicstr, 8);

	const char *argv[8] = {"evalsha", cfg("redis.shasadd"), "1", topic, sid};
